    ...
    def rotate_matrix(matrix, direction)
      ...
      oldMap.each do |oldLine|
        ...
        oldLine.each do
          case direction
          when :right
            self[(columnCount - 1) - onColumn][(lineCount - 1) - onLine] = oldLine[(columnCount - 1) - onColumn]
          when :left
            self[onColumn][onLine] = oldLine[(columnCount - 1) - onColumn]
          end
          onColumn += 1
        end
        onLine += 1
